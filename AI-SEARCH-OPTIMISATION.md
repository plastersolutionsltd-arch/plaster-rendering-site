...
| | **RESUME TOMORROW (21 Jun):** Roll the same answer-capsule out to the **48 suburb pages** (priority 4) — localised answer per suburb ("Looking for a plasterer in {Suburb}? P&R Solutions covers {S-postcode}…"). Do as a SCRIPTED BATCH using the entity-only method (see gotcha above): anchor on the `About …` eyebrow block, add `.answer-capsule` to each page's speakable selector. Then request indexing in GSC for /about + key updated pages. Service-page capsule prices are accurate as of today. |
