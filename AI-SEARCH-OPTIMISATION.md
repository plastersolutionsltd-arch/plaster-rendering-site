...
| Date | Action | Result |
|---|---|---|
| 9 Aug 2026 | **CACHING: `immutable` was stranding returning visitors for a year** | `/app.css` and `/images/(.*)` were served `max-age=31536000, immutable` while their filenames never changed — a promise the bytes never change, made about files that were repeatedly replaced. **72 assets swapped in place since May** under it: the 18 Jul recompression of 61 images and the 6 Aug logo recolour never reached anyone who had visited before those dates. It also caused a **live regression**: the 8 Aug font move put `@font-face` in app.css and removed the Google Fonts `<link>` from the HTML, so a returning visitor got new HTML + cached old CSS = **no webfonts at all** (verified: old app.css had 0 references to outfit-var.woff2, live HTML 0 Google Fonts links). FIX: app.css content-hashed (`app.212d8511.css`, 55 refs), keeping immutable now that the name changes with the bytes; `/images/` dropped to 1 day + 1 week stale-while-revalidate since those names are not hashed. Greenhill hero also content-hashed. **GOTCHA: `curl` proves what the ORIGIN sends, not what a browser shows** — Chris said the photo looked unchanged twice while curl said it had; the cache was the answer, not the file. Also written up in the new `HANDOVER.md`. |
| 6 Aug 2026 | **BRAND: one green — `#00FF00` — site and logos** | The brand had drifted into **three** greens and nothing matched anything: `#00FF00` in the site headings/buttons/links, **`#66FF33` in the raster logos actually displayed in the header** (`logo-transparent.webp` 73 pages, `logo-white.webp` 68 pages), and `#37D413` in the four SVG masters plus the site badges and section gradients. The raster and vector logos were exported from different sources at some point. Chris chose `#00FF00`. Site: 2,317 values folded in across 65 files, hover pair now `#00CC00`; `#37D413`/`#66FF33`/`rgba(55,212,19,...)` all now return **zero** matches. Logos recoloured pixel-by-pixel with the alpha channel preserved (no edge fringing, checked on dark and light grounds before installing). **GOTCHA: lossy WebP shifted the mark to `#00FF01`** — re-encoded lossless, which made the files *smaller* (8KB&rarr;6KB) and exact; verified by re-downloading from the live server. Standalone logo set + colour reference at `~/Projects/PR-Logos-00FF00` (plain folder, not version-controlled). **PRINT WARNING: `#00FF00` is outside CMYK gamut** — process printing comes back noticeably duller, so signage/vinyl/embroidery must match **Pantone 802 C** or a physical sample, and should use the SVGs. |
| 6 Aug 2026 | **Full-site audit + fix pass (65 pages, live-browser driven)** | **PRIVACY: `/AI-SEARCH-OPTIMISATION.md` and `/seo-ranking-tracker.md` were serving publicly at 200** (this file — full strategy, competitor names, GSC numbers) with robots.txt allowing all. Blocked via `.vercelignore` + explicit `vercel.json` redirects; verified live 308. **PERF: homepage mobile images 1268KB &rarr; 318KB** — the six service-card images are CSS backgrounds, which cannot use srcset, so phones got the desktop files; the `-m` variants already existed and were never referenced. Two `-m` files had themselves been built at 1512x2016 (regenerated 300&rarr;54KB, 196&rarr;60KB); logo 2584px&rarr;400px (54&rarr;8KB). **CONTRAST: hero headline 1.25:1 &rarr; 6.67:1** (AA large needs 3.0) — the 105deg scrim faded to .05&ndash;.15 under the right-hand end of a 115px headline, same class as the 28 Jul CTA overlays. **GOTCHA: `app.css` is a PRECOMPILED tailwind build** — only `text-white/50` and `/75` exist in it, so `/80` renders as NO colour at all; hero colour is now inline. `text-white/40` on the Sandygate case study was already broken this way. **REVIEWS: count drifted** (57 pages 105, 4 pages 104 — they used `"reviewCount":"104"` with no space so the 28 Jul sweep missed them — and /projects displayed 95). Now driven by `sync-reviews.py`, all 61 pages on **107** to match GBP. **Also:** mobile Get Quote restored on /pricing, /about, /how-we-apply (sat inside a desktop-only container); 44px tap targets on 363 controls (the header phone link was 15px tall); literal `&amp;` stripped from JSON-LD on 11 pages; nav labels normalised to "Projects" (was Portfolio/Gallery/More Projects); 2 long metas trimmed; /projects card scrims strengthened. **Clean:** 60/60 URLs 200 zero-redirect, 122/122 JSON-LD valid, 0 broken links, 0 mojibake, no sideways scroll at 390/1280/1440. **CORRECTION to an earlier assumption:** the images folder is 179MB locally but `.gitignore` excludes `images/**/*.jpeg`, so only 37MB deploys and just 7MB is unreferenced — untracked camera originals never reach production. **Resolved same day:** the greens — see the next row. **Still open:** the 2.8MB `.mov` plays in Chrome (verified) and needs ffmpeg to transcode. |
| 28 Jul 2026 | **Full-site visual + integrity audit (65 pages)** | Triggered by two real defects Chris spotted on `/about`: (1) the third "approved applicator" chip looked empty &mdash; `chas-logo.webp` is a **white logo on a transparent background** (68% transparent, avg opaque pixel RGB 254,254,254) inside a **white** chip, so it was rendering perfectly invisibly. Replaced with Johnstone's Stormshield (the logo the silicone + monocouche pages already use); CHAS was the odd one out under a "render systems" heading and still has its own trust card. (2) the CTA photo was buried under a flat `rgba(7,11,7,0.88)` overlay &rarr; gradient 0.70/0.60/0.64, and `object-position` raised from `center 80%` (parked on the paving) to `center 45%`. Same CTA on `how-we-apply`. **Audit then found the same overlay defect on 4 more pages** &mdash; pricing, plastering-cost, rendering-cost, silicone-vs-monocouche all at 0.94&ndash;0.95 alpha &rarr; 0.90/0.55/0.35. **GOTCHA: a 135deg hero gradient puts its LIGHT half under the heading at 390px** &mdash; on pricing.html the white "Transparent" landed on a sunlit wall, so all four gained a `max-width:640px` vertical overlay (0.86/0.72/0.66). **Clean:** 185 referenced images all on disk + 200 live, 122 JSON-LD blocks valid, 0 internal links to a missing page/route, no other white-on-white logos, and the 4 noindex preview/template pages are correctly excluded (3 of them 404 in production &mdash; no clean-URL rewrite). **Not touched deliberately:** 55 pages flagged for titles >65 chars &mdash; that's a naive char threshold, Google truncates on pixel width and they read fine. |
| 28 Jul 2026 | **Chapeltown page: real Google review (Dave Holmes) + site-wide review count** | Second location page to carry a real, verifiable review (after Walkley 18 Jul). Dave Holmes' 5-star review (posted 24 Jul — boarding + plastering after walls/chimney breast removed and steels installed in a Chapeltown kitchen/diner, plus a second room done while they were on holiday) added to `chapeltown-sheffield.html` as **visible card + matching `Review` schema** on the LocalBusiness node — Google requires the schema review to be visible on the page. Sign-off lines ("Thank you Chris… Dave & Maggie") trimmed so the visible quote and `reviewBody` match exactly. **GOTCHA: HTML entities inside a `<script type="application/ld+json">` block are NOT decoded** — they stay literal, so `&#39;` would ship as `wouldn&#39;t` in a rich snippet; use plain apostrophes in JSON-LD (JSON needs no escaping for `'`). Site-wide `reviewCount` **104 → 105** across 57 pages + the two visible counts on `rendering-sheffield.html` (the only page printing the number rather than "100+"). All 122 JSON-LD blocks re-validated, 0 invalid. Card verified rendered at 390px + 1280px, no horizontal overflow. |
...
| 18 Jul 2026 | **.com verified in GSC + Change of Address FILED** | TXT `google-site-verification=irUa…RGK1w` added to Vercel DNS via CLI (rec_b31fa68500dc055bbb5deff8), domain property verified, Change of Address submitted → GSC banner confirms ".com moving to www...co.uk". Google now actively migrates the ghost domain's history to the live site (180-day window). Also: first suburb review from owner shipped — Mark Beachell 5★ (11 Jul, EWI gable end, names Walkley) added to walkley-sheffield.html as visible card + Review schema per the Sandygate pattern, deployed + verified live. |
| 18 Jul 2026 | **/rendering-sheffield HUB PAGE LIVE** | New page for the biggest untapped keyword group ("rendering sheffield" 167 imp + render/renderer/rendering-near-me long tail, no page targeted it). Built from silicone-page chrome: answer-first capsule (cost-guide figures), render comparison TABLE (silicone/mono/sand-cement/EWI/repair w/ real prices, snippet-friendly), 3-step process, 3 REAL reviews (Scott Hill, Mark Beachell, Andy B.), 7-Q FAQ (all answers from existing published facts). Schema: adapted @graph (Service w/ review array, WebPage+speakable), FAQPage, BreadcrumbList, ItemList of 5 systems. Footer "House Rendering" link added on 55 pages; vercel.json rewrite + sitemap 0.9 added. All JSON-LD valid, zero mojibake, live-verified. |
| 18 Jul 2026 | **Hub page schema fix + INDEXING REQUESTED** | GSC live test flagged "7 review items, some invalid": cloning silicone chrome had (a) carried 3 INVISIBLE LocalBusiness reviews (Edward/Tom/Richard — not shown on hub page = guideline breach) and (b) put the 3 visible reviews on the Service node — Service is NOT a valid review-snippet type. FIX: visible reviews (Scott Hill/Mark Beachell/Andy B.) moved onto LocalBusiness node, Service review array removed — now mirrors site-wide pattern, 4 valid items. Deployed + live-verified. Owner re-ran live test + clicked Request Indexing same day. LESSON: when cloning a page's schema graph, strip inherited review arrays — every page's reviews must match its own visible cards. |
...
| 23 Aug 2026 | First review added since the corpus was captured | Leamne Shortland (5★, 22 Aug) added to **/rotherham-south-yorkshire** — visible card + `Review` node on LocalBusiness. Job was Thorpe Hesley **S61** (Rotherham borough); no Thorpe Hesley page exists and the Rotherham page had **zero** reviews, so it gains most. Thorpe Hesley also added to the page copy (FAQ answer in JSON-LD **and** visible — keep in step — plus the area paragraph); it was named nowhere before. Review count **108→109** via `sync-reviews.py` across 62 pages. Homepage review-ask rewritten to ask customers to name the **work** and the **area**, with a worked example. **Caveat:** the review text itself does not name the area (owner supplied it), so it does not move "Rotherham"/"Thorpe Hesley" — Google reads the review text. It does name both services ("outdoor render and internal plastering"). Still **0 of 99** say monocouche. Published full-postcode-free (S61, not S61 2UD) — never publish a customer's full postcode. |
| 23 Aug 2026 | **Off-site reviews + freshness + llms.txt** | **Checkatrade** (2, both 10/10) and **Yell** (10) pasted by Chris; all in new `offsite-reviews-source.md`. Checkatrade attaches a **job location** itself &mdash; the only reviews in any corpus with a location we did not supply. Both are **Barnsley** (S70 town, S73 Wombwell/Darfield): site has a footer mention + `areaServed` claim but **no Barnsley page**. Placed: S73 plastering &rarr; /plastering-sheffield, S70 rendering &rarr; /rendering-sheffield, 3 Yell wall reviews &rarr; /sand-cement-render-sheffield (had zero). **Andy = AndyB-3248, a YELL review not Google** &mdash; was shown with a Google G badge under two different names; fixed + removed from schema (it sat inside an aggregateRating counting 109 *Google* reviews). **Freshness:** all 58 pages claimed dateModified 21 Jun or earlier while edited through Aug (dry-lining 160 days out); new `sync-dates.py` reads each page's real last substantive change from git, skipping review-count-only commits (bumping all dates = date-spoofing). **llms.txt** added (marginal: crawlers skip it, no Google Search system reads it, uplift only on Claude/Perplexity) &mdash; every price verified against live pages; count slot added to `sync-reviews.py`. |
| 23 Aug 2026 | **&#9888; SCHEMA ELIGIBILITY &mdash; the review markup is not doing what we thought** | Google's own docs, two pages: review-snippet guidance says *"Don't aggregate reviews or ratings from other websites"*, and LocalBusiness says `review`/`aggregateRating` are *"only recommended for sites that capture reviews about other local businesses"*. So (a) Yell/Checkatrade reviews **can never** go in JSON-LD, and (b) the ~30 **Google** review nodes across 13 pages are **self-serving and ineligible for star snippets** &mdash; they have not been earning stars and will not. Ineligible &ne; penalised, so nothing is on fire and they were left in place. **Decision pending:** check what GSC actually reports for them before stripping. The *visible* cards keep their value regardless &mdash; AI Overviews read page content, not just markup. |
| 23 Aug 2026 | **&#9733; THE PRIORITY, from 2026 research** | **AI citation threshold is ~150 Google reviews per location** &mdash; below it, AI Overviews/ChatGPT/Perplexity/Gemini rarely name a business as a recommendation. **We are on 109.** **Review recency moved #93 (2023) &rarr; #11 (2026)**, the largest jump in the study. Signal weights: **GBP 32%, on-page 19%, reviews 16%, links 15%, behavioural 8%, citations 7%.** The website is the 19% and is already strong; **~41 more Google reviews is the highest-value action available** and it is owner-driven. Order: (1) reviews to 150, (2) GBP, (3) off-site citations (TrustATrader 01438 870097, Which?, TrustMark, Weber form) &mdash; still unstarted and still the tracker's long-standing #1, (4) Barnsley page **only if GSC shows demand**. Sources: Whitespark AI-Overviews local study; PinMeTo 2026 local ranking factors. |
| 24 Aug 2026 | **GSC data read + NEW PAGE: /render-over-pebbledash-sheffield** | **Generative AI report (Beta): 2.44K AI impressions in 3 months**, ~11% of total, and climbing hard &mdash; ~5-10/day in May to a **110/day peak 12-14 Aug**. Top AI pages: plastering-sheffield 405, homepage 304, dry-lining 300, faq 197, silicone-vs-monocouche 169, how-we-apply 167, monocouche 162, render-repair 157. **The homepage is 2nd in AI but 58% of clicks in normal search** &mdash; AI rewards question/comparison/process content, not landing pages. That is now evidenced, not assumed. **Corrected an earlier over-claim of mine:** the ~150-review AI threshold is about being *named as a recommendation*; the site is already surfacing in AI features at 109 reviews. **Web perf 3mo: 262 clicks / 22.2K impressions / 1.2% CTR / pos 21.5** &mdash; impressions **doubled** vs the 11.1K baseline, clicks +37%, **position flat**. 28-day: 104 clicks (+41%), 8.72K impr (+23%), so it is accelerating. Diagnosis: huge visibility, position problem &mdash; on-page is working, position needs reviews/GBP/citations. **Barnsley REJECTED on data: 6 impressions, 0 clicks in 3 months** (asked before building). **Built instead: /render-over-pebbledash-sheffield** &mdash; 51 pebbledash mentions were scattered across 8 pages with no hub, same shape as "rendering sheffield" pre-July; Checkatrade/MyJobQuote/CostShed all run dedicated pebbledash cost pages. Method text is Chris's own from /faq + /how-we-apply; render prices are the site's own published figures; the removal range (&#163;2,100-&#163;7,800) is labelled as third-party UK guides, NOT his price list. Built off the silicone-vs-monocouche shell. **Two faults caught by checking, not assuming: 20 of 45 CSS classes in the first draft did not exist in that inline stylesheet (would have rendered as nothing), and the copied hero was missing `id="heroImg"` causing a null-classList JS error on every load.** Rewrite + sitemap + 4 topical inbound links + llms.txt entry. **OWNER ACTION: request indexing for /render-over-pebbledash-sheffield in GSC.** |
| 24 Aug 2026 | **Redirect error FIXED + tap targets root-caused** | **GSC "Redirect error: 2"** traced live: all 3 dev preview pages returned `308` to a clean URL with no rewrite, so the chain ended on a **404**. Both forms now redirect to `/`, placed BEFORE the generic `/:slug.html` stripper (vercel evaluates in order), and the files are `.vercelignore`d. **Ruled out while diagnosing:** all 62 sitemap URLs return 200, are indexable and self-canonicalise; `.html` variants correctly 308 so there is no duplicate split; only 1 page is thinly linked. So the **8 "Crawled - currently not indexed"** are not technical &mdash; they are a content-value judgement, and the only content anomaly is the suburb pages. **Suburb duplication measured: 87.9% average similarity with the suburb name neutralised; Beighton vs Mosborough 98.2%, Southey vs Wisewood 97.7%.** Google's spam policy names this pattern verbatim ("pages targeted at specific regions or cities", "substantially similar pages"). **Strong hypothesis for flat position, NOT proven, and no manual action exists &mdash; 41 live pages not touched.** **Tap targets root-caused:** `.tap44` had **no min-height** while `.tap44-btn` did; **211 elements across 42 pages** only looked fixed. Also found **12 pages do not link app.css at all** (self-contained inline-style pages) so an app.css fix reaches 55/67 &mdash; rules duplicated into those 12. Announcement-bar phone link was **15px on 55 pages**, now 44px. Verified by measurement: 56 targeted controls, 0 under 44px. Known open: broomhill overflows 10px (nested padding, cosmetic). |
| 24 Aug 2026 | **FULL SITE AUDIT (64 live pages)** | **Fixed:** (1) `/get-quote` &mdash; the highest-intent page &mdash; was loading **6 manufacturer logos from static.wixstatic.com**, a third-party CDN left from the old Wix site, two of them 215KB/224KB JPEGs. Localised to webp: **443KB &rarr; 23KB**, dependency gone. (2) Those 6 had no width/height (layout shift); now set. (3) **9 pages preloaded the desktop hero unconditionally** while `<picture>` served the `-m` variant, so phones downloaded both &mdash; the same bug fixed on 29 pages in August, these were missed. Media-scoped. 11 others have no `-m` variant to scope to. (4) 3 Article nodes had no `image`; added a representative hero (the automated pass first picked the logo &mdash; valid schema, useless to Google). (5) **Business-card PDFs (773KB x3) were publicly served**; now `.vercelignore`d, verified 404. (6) My own pebbledash meta description was 177 chars; now 146. **Two of my own claims corrected:** the "2.7MB deployed png" is gitignored and 404s live (I measured excluding only `.jpeg`) &mdash; real deployed image weight is **35.8MB/265 files**; and the "missing alt" on S17 is inside an **HTML comment**, so zero images lack alt. **Clean across 64 pages:** no JS errors, no 4xx/5xx, no broken internal links, JSON-LD valid, no duplicate titles/descriptions, 1 H1 each, no mojibake. **Open, not fixed:** broomhill overflows 10px (nested padding, cosmetic); 11 heroes lack `-m` variants. |
| 24 Aug 2026 | **&#10007; Suburb-duplication theory TESTED AND DROPPED** | Measured 41 suburb pages at **87.9% average similarity** (Beighton vs Mosborough 98.2%) and Google's spam policy names that pattern verbatim. It fitted the symptom exactly &mdash; impressions doubling, position pinned at 21.5. **Chris ran URL Inspection on /mosborough-sheffield and /wisewood-sheffield: BOTH INDEXED.** Google would not index them if it treated them as doorway pages. **Theory dropped; the 41 pages were NOT rewritten.** Two minutes of checking saved a 41-page rewrite &mdash; repeat that pattern. Combined with the same-day finding that all 62 sitemap URLs are technically clean, **the site has no technical SEO fault and is exhausted as a lever.** Position is off-page: reviews (109, need ~150), GBP (32% of weight), links/citations (22%). |
| 24 Aug 2026 | **CONTENT PUSH: 3 new pages, all indexed-requested** | Topics chosen from the AI report (informational content earns citations, not landing pages). **(1) /render-over-pebbledash-sheffield** &mdash; 51 mentions across 8 pages with no hub; method text is Chris's own, removal range labelled as third-party guides not his prices. **(2) /planning-permission-render-sheffield** &mdash; Planning Portal quoted verbatim + Sheffield's **35 conservation areas** from the council's own list, 11 linking to existing suburb pages; Article 4 on Nether Edge (2005) + Broomhill. **(3) /rendering-in-winter-sheffield** &mdash; **&#9733; the big one.** Chris mentioned he sprays **Ecorend SR365** in the cold; verified with the manufacturer: rated **&#8722;5&#176;C to +25&#176;C, humidity to 99%, rainproof 2hrs above +5&#176;C**, where **every other render stops at +5&#176;C**. **SR365 appeared ZERO times on the site** and the FAQ understated it. His own cold-weather video embedded (poster from the clip, preload=none verified, VideoObject schema). **Trap hit:** updating an FAQ needs BOTH the JSON-LD and the visible copy &mdash; the visible version uses `<strong>` + HTML entities so a plain-text replace updated schema only, briefly leaving index.html contradicting itself. **Open, pre-existing:** 7 FAQ answers on /faq and 3 on index are in schema but NOT visible on the page. |
| 25 Aug 2026 | **FAQ schema aligned to the visible copy + alt text audit** | Closed the 24 Aug open item. I had **OVERSTATED it** &mdash; called it a Google guideline breach; it was mostly **text drift**, the page wording having moved on while the schema stayed put. Measured properly (entities, curly quotes and dashes normalised &mdash; **exact string comparison reported 91 broken when the real figure was far smaller**): questions not on the page **30 &rarr; 0**, answer drift **77 &rarr; 17**. Fixed by realigning **21 questions and 62 answers to the visible copy** (the page is the source of truth) and removing **9 schema questions with no visible counterpart**. The 17 remaining are wording-only ("thickness, temperature" vs "thickness, room temperature") and deliberately left. **Harness lesson:** a fix script that `break`s after the first JSON-LD block silently does nothing on pages where FAQPage is not first &mdash; mine reported "0 aligned, 0 extra", a check that could not fail, caught only because zero contradicted the 30 found minutes earlier. **Alt text:** 355 images, **0 missing, 0 empty, 0 repeating the town name**. Trimmed 14 that ran 126-178 chars because a promotional tail ("by P&amp;R Solutions Sheffield plasterers") had been appended to a good description; fixed a real templating bug on `s17-ewi-case-study` where four captions read "&hellip; S17 Sheffield, S17 Sheffield". **Nine over 125 chars left deliberately** &mdash; genuinely descriptive, and 125 is a guideline not a limit. |
| 25 Aug 2026 | **AUDIT + FIXES: tap targets on the self-contained pages, freshness, llms.txt** | Branch `fix/tap-targets-freshness-25aug`, **not merged, not deployed**. **Verified clean and NOT to be re-run:** 0 missing titles/canonicals, 0 duplicate titles or descriptions, 0 overlong descriptions, **125/125 JSON-LD blocks valid**, 1 H1 per page, review count 110 everywhere, sitemap &harr; files &harr; `vercel.json` rewrites fully consistent, no broken images, no 4xx, no JS errors. **The real find:** the 24 Aug `.tap44` fix did not end the tap-target problem &mdash; **it inverted it.** On the **10 pages that do not link app.css**, `.tap44-btn` is *used but never defined*, so on `/planning-permission-render-sheffield` **12 conservation-area links carried the 44px class and rendered at 39px** &mdash; on a page built the same day as the fix. Also **`.btn-call`, the `tel:` link, at 29px on 6 pages** including both cost pages (the primary conversion control on a trades site), and **`.nav-burger` at 28px on 7**. Root cause of the recurrence: new pages are cloned from the `silicone-render-vs-monocouche` shell, which is itself self-contained, so every new page inherits the blind spot. All now &ge;44px, measured at 390px and 1280px before and after; **no page changed scrollWidth**, only the 15-20px the taller tel: link adds. **`sync-dates.py` had never been re-run after the 25 Aug commits &mdash; 19 pages claimed 8 Aug.** Fixed. **`llms.txt` was missing 2 of the 3 new pages** (only pebbledash got an entry); added under a new `## Guidance` heading, 20 &rarr; 22 URLs. **Open, found not fixed:** `/rotherham-south-yorkshire` has **ONE inbound internal link** &mdash; with Beighton and Meersbrook having sat unindexed on 5-7 links, the thinnest-linked pages are the best candidates for the **8 GSC "crawled, not indexed"** (rotherham 1; lodge-moor, parson-cross, plastering-cost, rendering-in-winter 3 each). FAQ answers on the self-contained pages are capped `max-height:400px` and planning-permission's tallest is **363px &mdash; 91% of the cap**, so one more sentence truncates silently. **413 FAQ buttons have no `aria-expanded`** (two accordion implementations exist). **Content gaps measured the pebbledash way:** **damp 83 mentions / 32 pages with no hub** (bigger than pebbledash's 51/8), **artex removal 36/6**, and **K-Rend on 5 pages / 7 mentions vs Ecorend's 12 / 79** despite being an approved applicator. **Corrections:** "11 heroes lack `-m` variants" is **10**, and only `monocouche-render-sheffield` (**164KB**) is heavy enough to matter &mdash; the rest are 16-76KB. `broomhill` remains the ONLY page of 64 that overflows, still exactly 10px. |
